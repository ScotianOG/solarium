...
import { ArrowDownUp, RefreshCw, Info, Zap } from "lucide-react"
...
export function SOLessSwiftSwap() {
  ...
  const [protocol, setProtocol] = useState<'solana' | 'sonic'>('sonic')
  ...
  const handleSwap = async () => {
    try {
      ...
      if (protocol === 'solana') {
        ...
        console.log(`Swapping ${amount} ${fromToken} to ${toToken} using Solana native${useGaslessMode ? " (Gasless Mode with " + gasToken + ")" : ""}`)
      } else {
        // Here you would call executeSonicSwap() from sonic-service
        console.log(`Swapping ${amount} ${fromToken} to ${toToken} using Sonic DEX${useGaslessMode ? " (Gasless Mode with " + gasToken + ")" : ""}`)
      }
      
      toast({
        title: "Swap Successful",
        description: `Swapped ${amount} ${fromToken} to ${estimatedAmount} ${toToken}${useGaslessMode ? " using gasless mode" : ""}`,
      })
  ...
  // Calculate fees
  const fees = {
    solana: 0.0005, // 0.05% example fee
    sonic: 0.0003,  // 0.03% example fee
    gasless: 0.002  // 0.2% additional fee for gasless swaps
  }
  
  const getTotalFees = () => {
    const baseFee = protocol === 'solana' ? fees.solana : fees.sonic
    return useGaslessMode ? baseFee + fees.gasless : baseFee
  }
  
  const feePercentage = (getTotalFees() * 100).toFixed(2)
  ...
  return (
    <Card className="bg-[#161616] border-gray-800">
      <CardHeader>
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <CardTitle>SwiftSwap</CardTitle>
            <Badge className="bg-cyan-600 hover:bg-cyan-700 text-white">
              SOLess
            </Badge>
          </div>
          <Tabs defaultValue="sonic" onValueChange={(value) => setProtocol(value as 'solana' | 'sonic')}>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Gasless Mode Toggle */}
        <div className="flex items-center justify-between bg-[#121212] p-3 rounded-md">
          <div className="flex items-center gap-2">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="max-w-xs">
                    Pay transaction fees in the token you're swapping instead of SOL
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        {useGaslessMode && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-amber-500">
              Note: Using gasless mode incurs an additional {(fees.gasless * 100).toFixed(2)}% fee
            </p>
          </div>
        )}
